...
function classifyModule(metrics, apiInfo, isInCycle) {
    ...
    if (fanout >= 10 || dependency_depth >= 5 || isInCycle) { // isInCycle makes it volatile by default if not caught by IMMUTABLE
        return "VOLATILE";
    }
...
function generateRecommendation(classification, moduleName) {
    ...
    if (classification.startsWith("VOLATILE")) {
        const reason = classification.includes("Cyclic") ? "its involvement in dependency cycles" : "its high fan-out or dependency depth";
...
async function processModule(moduleName, moduleFilePath, circularDepsData) {
    try {
        ...
        const genDocstringPath = path.join(GENERATED_DOCSTRING_INVENTORY_DIR, `${sanitizedModuleName}_generated_docstrings.json`);
        ...
        if (!moduleDocstring) { // Fallback if module-specific docstring is still null
            const classNames = Object.keys(apiContract.classes || {});
            if (classNames.length === 1) { // If only one class, use its docstring
                 const className = classNames[0];
                 if (originalDocstrings?.elements) {
                    const classDoc = originalDocstrings.elements.find(e => e.element_type === "class" && e.name === className);
                    if (classDoc && classDoc.docstring) moduleDocstring = classDoc.docstring;
                 }
                 if (!moduleDocstring && generatedDocstrings?.elements) {
                    const genClassDoc = generatedDocstrings.elements.find(e => e.element_type === "class" && e.name === className);
                    if (genClassDoc && genClassDoc.generated_docstring) moduleDocstring = genClassDoc.generated_docstring;
                 }
            }
        }
        ...
        return {
            ...
            recommendation,
            in_cycle: isInCycle,
            interface_count: apiContract.api_summary?.interface_count || Object.keys(apiContract.interfaces || {}).length,
...
async function main() {
    ...
    try {
        ...
        const outputFilePath = path.join(SAFE_MUTATIONS_OUTPUT_DIR, "safe_mutations_report.json");
        utils.writeJsonFile(outputFilePath, validResults, 2, true);

        console.log(`Safe mutations analysis complete. Report at: ${outputFilePath}`);
