	...
	visitNode(node, parentName) {
		if (ts.isClassDeclaration(node) && node.name) {
			...
			if (node.body && ts.isModuleBlock(node.body)) {
				ts.forEachChild(node.body, (child) => this.visitNode(child, parentName ? `${parentName}.${namespaceName}` : namespaceName));
			}
...
function generateIndexFile(results, targetDir) {
	try {
		...
		utils.writeJsonFile( // writeJsonFile is from utils.js
...
async function main() {
	...
	try {
		...
		const excludeDirs = excludeDirsArgIndex !== -1 ? args.slice(excludeDirsArgIndex + 1) : utils.DEFAULT_EXCLUDE_DIRS;
