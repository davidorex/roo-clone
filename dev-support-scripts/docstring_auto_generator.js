...
function generateFunctionMethodDocstring(elementName, apiElement, elementType) {
    ...
    const commonVerbsMap = {
		get: "Retrieves", fetch: "Fetches", load: "Loads",
		set: "Sets", update: "Updates", modify: "Modifies",
		is: "Checks if", has: "Determines if", can: "Checks whether",
		create: "Creates", add: "Adds", build: "Builds",
		...
		parse: "Parses",
		submit: "Submits",
		start: "Starts", init: "Initializes",
		stop: "Stops", abort: "Aborts",
		save: "Saves", store: "Stores",
		execute: "Executes", run: "Runs",
		ensure: "Ensures",
		format: "Formats",
		present: "Presents",
    ...
    const objectForSentence = objectCandidate.replace(/([A-Z])/g, ' $1').toLowerCase().trim();
	...
	} else { // Fallback for unmatched verbs
        actionStatement = `Handles the ${elementName.replace(/([A-Z])/g, ' $1').toLowerCase().trim()} operation.`;
    }
	...
	if (apiElement.parameters && apiElement.parameters.length > 0) {
		apiElement.parameters.forEach(param => {
			let paramDesc = `The ${param.name.replace(/([A-Z])/g, ' $1').toLowerCase().trim()}`;
	...
	if (apiElement.return_type && apiElement.return_type !== "any" && apiElement.return_type !== "void" && !apiElement.return_type.endsWith("Promise<void>") && !apiElement.return_type.endsWith("Promise<undefined>")) {
...
function generateClassDocstring(className, apiElement, dependenciesData) {
    ...
    if (role) { // If a role was found via suffix, try to get a cleaner object name
        for (const suffix in suffixes) {
            if (className.endsWith(suffix)) {
    ...
    objectName = objectName.replace(/([A-Z])/g, ' $1').toLowerCase().trim();
    ...
    const propertyTypesAndNames = apiElement.attributes ? 
        Object.entries(apiElement.attributes).map(([name, type]) => ({name: name.toLowerCase(), type: String(type).toLowerCase()})) : [];
    ...
    if (methodNames.some(m => m.toLowerCase().includes("api") || m.toLowerCase().includes("request"))) themes.add("API interactions");
    if (methodNames.some(m => m.toLowerCase().includes("message") || m.toLowerCase().includes("webview") || m.toLowerCase().includes("ui"))) themes.add("UI/webview communication");
    if (methodNames.some(m => m.toLowerCase().includes("tool"))) themes.add("tool execution");
    if (methodNames.some(m => m.toLowerCase().includes("state") || m.toLowerCase().includes("history") || m.toLowerCase().includes("settings") || m.toLowerCase().includes("config"))) themes.add("state and configuration");
    ...
    if (methodNames.some(m => m.toLowerCase().includes("checkpoint") || m.toLowerCase().includes("git"))) themes.add("version control and checkpoints");
    if (methodNames.some(m => m.toLowerCase().includes("file") || m.toLowerCase().includes("directory"))) themes.add("file system operations");
    if (methodNames.some(m => m.toLowerCase().includes("event") || m.toLowerCase().includes("listener"))) themes.add("event handling");


    // Property-based themes
    if (propertyTypesAndNames.some(p => p.type.includes("cline") && !p.name.includes("provider"))) themes.add("Cline instance orchestration"); // Avoid self-reference for ClineProvider
...
function generateInterfaceDocstring(interfaceName, apiElement) {
    let desc = `Defines the contract for a ${interfaceName.replace(/([A-Z])/g, ' $1').toLowerCase().trim()}.`;
...
function generatePropertyDocstring(propertyName, apiProperty) {
    ...
    let desc = `The ${propertyName.replace(/([A-Z])/g, ' $1').toLowerCase().trim()}`;
    const lowerPropName = propertyName.toLowerCase();
    const humanizedName = propertyName.replace(/([A-Z])/g, ' $1').toLowerCase().trim();

    if (lowerPropName.endsWith("id") || lowerPropName.endsWith("identifier")) {
        desc = `The ${humanizedName} identifier`;
    } else if (lowerPropName.startsWith("is") || lowerPropName.startsWith("has") || lowerPropName.startsWith("did") || lowerPropName.startsWith("can") || lowerPropName.startsWith("should") || type === "boolean") {
    ...
    } else if (lowerPropName.includes("setting")) { // handles singular "setting" too
        desc = `The ${humanizedName} settings`;
    } else if (lowerPropName.includes("history")) {
        desc = `The ${humanizedName} history`;
    } else if (lowerPropName.includes("message") && !lowerPropName.includes("errormessage")) { // avoid "error message message content"
        desc = `The ${humanizedName} message content`;
    } else if (type && type !== "any") {
        const typeLower = String(type).toLowerCase();
        if (lowerPropName.includes(typeLower.replace(/<.*>/, '').trim())) { // if prop name contains base type name e.g. terminalManager: TerminalManager
...
function generateTypeAliasDocstring(typeName, apiType) {
    ...
    if (apiType.values && apiType.type === "union") { // from api_contract_analyzer for type aliases
...
async function analyzeAndGenerateForFile(moduleDocstringsPath, moduleApiContractsPath, moduleDependenciesPath) {
	try {
		...
		for (const extractedElement of docstringsData.elements) {
			if (extractedElement.docstring === null) { // Only process if original docstring is missing
				let apiElement;
                ...
                switch (extractedElement.element_type) {
                    case "class":
                        ...
                        if (apiElement) generatedDocstring = generateClassDocstring(extractedElement.name, apiElement, dependenciesData);
                        break;
                    case "method":
                    case "constructor":
                    case "getter":
                    case "setter":
                        apiElement = apiContractsData.classes?.[extractedElement.parent_name]?.methods?.[extractedElement.name];
                        if (apiElement) generatedDocstring = generateFunctionMethodDocstring(extractedElement.name, apiElement, extractedElement.element_type);
                        break;
                    case "function":
                        apiElement = apiContractsData.functions?.[extractedElement.name];
                        if (apiElement) generatedDocstring = generateFunctionMethodDocstring(extractedElement.name, apiElement, "function");
                        break;
                    case "interface":
                        apiElement = apiContractsData.interfaces?.[extractedElement.name];
                        if (apiElement) generatedDocstring = generateInterfaceDocstring(extractedElement.name, apiElement);
                        break;
                    case "property": // Can be class attribute or interface property
                        if (extractedElement.parent_name && apiContractsData.classes?.[extractedElement.parent_name]?.attributes) {
                            apiElement = apiContractsData.classes[extractedElement.parent_name].attributes[extractedElement.name];
                        } else if (extractedElement.parent_name && apiContractsData.interfaces?.[extractedElement.parent_name]?.properties) {
                            apiElement = apiContractsData.interfaces[extractedElement.parent_name].properties[extractedElement.name];
                        }
                        if (apiElement) generatedDocstring = generatePropertyDocstring(extractedElement.name, apiElement);
                        break;
                    case "type_alias":
                        apiElement = apiContractsData.types?.[extractedElement.name];
                        if (apiElement) generatedDocstring = generateTypeAliasDocstring(extractedElement.name, apiElement);
                        break;
...
async function workerThread() {
	...
	for (const fileTuple of files) {
		const result = await analyzeAndGenerateForFile(fileTuple.docstringsFile, fileTuple.apiContractsFile, fileTuple.dependenciesFile);
		if (result) {
			results.push(result);
		}
	}
...
async function main() {
	...
	try {
		...
		const fileTuplesToProcess = docstringsIndex.modules.map(mod => {
            ...
            try {
                // Ensure all three required files exist before attempting to process
                fs.access(fileTuple.apiContractsFile); 
                fs.access(fileTuple.dependenciesFile);
                return true;
            } catch {
                // console.warn(`Skipping ${fileTuple.docstringsFile}, missing corresponding API contract or dependency file.`);
                return false;
            }
        });
		...
		for (const moduleInfo of validResults) {
			...
			utils.writeJsonFile(
				path.join(GENERATED_DOCSTRING_OUTPUT_DIR, filename),
				moduleInfo,
				2,
				true,
			);
