# Querying the Git Commit Viewer Database for cline Commits

This guide explains how to query the `git_commit_viewer` PostgreSQL database to retrieve commit information for the cline repository using the newly installed PostgreSQL MCP server.
...
### Get cline Repository ID

```sql
SELECT id, name, path FROM viewer_repository WHERE name = 'cline';
```

### Retrieve Recent cline Commits

This query fetches the 10 most recent commits from the cline repository:
...
Get cline commits within a specific date range:
...
JOIN viewer_commit_branches cb ON c.hash = cb.commit_id
JOIN viewer_branch b ON cb.branch_id = b.id
WHERE b.repository_id = (SELECT id FROM viewer_repository WHERE name = 'cline')
...
Count the number of commits per branch in cline:
...
<arguments>
{
  "sql": "SELECT c.hash, c.short_message, c.author_name, c.commit_date FROM viewer_commit c JOIN viewer_commit_branches cb ON c.hash = cb.commit_id JOIN viewer_branch b ON cb.branch_id = b.id WHERE b.repository_id = (SELECT id FROM viewer_repository WHERE name = 'cline') ORDER BY c.commit_date DESC LIMIT 10;"
}
</arguments>
</use_mcp_tool>
```
