...
This document outlines a simple, elegant approach for using Python-based analysis scripts with TypeScript codebases, specifically within the Cline and DaFu projects. By leveraging the existing tree-sitter implementation and maintaining a clean separation of concerns, we achieve a system that combines the strengths of both ecosystems without complicating the VSIX build process.

Ideally this plan will be synthesized with the ideas in dev-support-scripts/May 12 Architecture for a language-agnostic suite.md
