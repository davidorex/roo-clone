# Mutations for src/core/tools/insertContentTool.ts
...
**File**: `src/core/tools/insertContentTool.ts`
**Context for Insertion**: This new block of code should be inserted immediately _after_ line 138 `cline.didEditFile = true` and _before_ line 140 `if (!userEdits)` block.
...
=======
		...
		if (cline.pauseAfterProductiveOperation) {
			...
			if (shouldPause) {
				await cline.say("operation_completed", "Content insertion completed successfully.")
...
- It occurs after the successful content insertion operation (`cline.didEditFile = true`)
